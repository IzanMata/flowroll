...
from factories import (AcademyFactory, AcademyMembershipFactory,
                       AchievementFactory, AthleteProfileFactory,
                       CheckInFactory, DojoTabTransactionFactory,
                       MatchupFactory, MembershipPlanFactory,
                       OpenMatSessionFactory, PromotionRequirementFactory,
                       SeminarFactory, SeminarRegistrationFactory,
                       SparringNoteFactory, SubscriptionFactory,
                       TimerPresetFactory, TrainingClassFactory, UserFactory,
                       VideoLibraryItemFactory)
from learning.models import (ClassTechniqueJournal, SparringNote,
                             VideoLibraryItem)
from matches.models import Match, MatchEvent
from membership.models import (DojoTabBalance, DojoTabTransaction,
                               MembershipPlan, PromotionRequirement, Seminar,
                               SeminarRegistration, Subscription)
from tatami.models import Matchup, TimerPreset, TimerSession, WeightClass
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--count", type=int, help="Override default record counts per model"
        )
    ...
    def handle(self, *args, **options):
        ...
        if fresh:
            self.stdout.write(
                self.style.WARNING("⚠️  FRESH MODE: This will delete ALL existing data!")
            )
        ...
        try:
            with transaction.atomic():
                ...
                self._seed_techniques_and_learning(
                    counts["techniques"], counts["videos"]
                )
                self._seed_attendance(academies, athletes, counts["classes"])
    ...
    def _seed_catalog_data(self):
        ...
        try:
            ...
            self.stdout.write(
                f"   ✅ Loaded {belts} belts, {techniques} techniques, {categories} categories"
            )

        except Exception as e:
            self.stdout.write(
                f"   ⚠️  Fixture loading failed ({e}), continuing with factory data"
            )
            # Fallback: create minimal belt data
            if not Belt.objects.exists():
                for i, color in enumerate(
                    ["white", "blue", "purple", "brown", "black"], 1
                ):
    ...
    def _seed_academies(self, count):
        ...
        for i in range(count):
            ...
            academy = AcademyFactory(name=f"{city_name} {academy_type}", city=city_name)
    ...
    def _seed_users_and_memberships(self, academies, count):
        ...
        for i in range(count):
            ...
            selected_academies = random.sample(
                academies, min(academy_count, len(academies))
            )

            for academy in selected_academies:
                # Role distribution: 80% student, 15% professor, 5% owner
                role_choice = random.choices(
                    ["STUDENT", "PROFESSOR", "OWNER"], weights=[80, 15, 5]
                )[0]

                AcademyMembershipFactory(user=user, academy=academy, role=role_choice)

        memberships = AcademyMembership.objects.count()
        self.stdout.write(
            f"   ✅ Created {len(users)} users with {memberships} academy memberships"
        )
        return users
    ...
    def _seed_athletes(self, academies, users, count):
        ...
        for i in range(min(count, len(users))):
            ...
            belt = random.choices(
                list(belt_weights.keys()), weights=list(belt_weights.values())
            )[0]

            # Stripes: weighted toward lower counts
            if belt != "black":  # Black belts use degrees, not stripes
                stripes = random.choices([0, 1, 2, 3, 4], weights=[30, 25, 20, 15, 10])[
                    0
                ]
            else:
                stripes = 0
            ...
            is_likely_female = (
                user.first_name in fake.first_name_female()
            ...
            athlete = AthleteProfileFactory(
                ...
                role=(
                    "PROFESSOR"
                    if membership.role in ["PROFESSOR", "OWNER"]
                    else "STUDENT"
                ),
            )
    ...
    def _seed_attendance(self, academies, athletes, class_count):
        ...
        self.stdout.write(
            f"📅 Creating {class_count} training classes with attendance..."
        )
        ...
        for academy in academies:
            ...
            for i in range(academy_class_count):
                ...
                class_date = fake.date_time_between(
                ...
                attending_athletes = random.sample(
                    academy_athletes, int(len(academy_athletes) * attendance_rate)
                )

                techniques_drilled = list(
                    Technique.objects.order_by("?")[: random.randint(2, 5)]
                )
                ...
                for technique in techniques_drilled:
                    ClassTechniqueJournal.objects.create(
                        ...
                        technique=technique,
                        professor_notes=(
                            fake.sentence() if random.random() < 0.4 else ""
                        ),
                    )
                    journal_entries += 1

        self.stdout.write(
            f"   ✅ Created {len(classes)} classes, {checkins_created} check-ins, {journal_entries} technique logs, {sparring_notes} sparring notes"
        )
        return classes
    ...
    def _seed_tatami_and_matches(self, academies, athletes, matchup_count):
        ...
        if not WeightClass.objects.exists():
            weight_classes = [
                ...
                ("Rooster", 57.5, 57.5, "M"),
                ("Light Feather", 64.0, 64.0, "M"),
                ("Feather", 70.0, 70.0, "M"),
                ("Light", 76.0, 76.0, "M"),
                ("Middle", 82.5, 82.5, "M"),
                ("Medium Heavy", 88.5, 88.5, "M"),
                ("Heavy", 94.5, 94.5, "M"),
                ("Super Heavy", 100.5, 120.0, "M"),
                # Female divisions
                ("Light Feather", 53.5, 53.5, "F"),
                ("Feather", 58.5, 58.5, "F"),
                ("Light", 64.0, 64.0, "F"),
                ("Middle", 69.0, 69.0, "F"),
                ("Medium Heavy", 74.0, 74.0, "F"),
                ("Heavy", 79.5, 100.0, "F"),
            ]
        ...
        for academy in academies:
            ...
            for _ in range(academy_matchup_count):
                ...
                suitable_opponents = [
                    a
                    for a in academy_athletes
                    if (
                        a != athlete_a
                        and abs(a.weight - athlete_a.weight) <= 10  # noqa: W503,W504
                        and a.belt == athlete_a.belt
                    )  # Within 10kg, same belt
                ]
                ...
                weight_class = (
                    random.choice(suitable_weights) if suitable_weights else None
                )
                ...
                if matchup.status == "COMPLETED" and random.random() < 0.7:
                    match = Match.objects.create(
                        ...
                        winner=random.choice(
                            [athlete_a.user, athlete_b.user, None]
                        ),  # Can be draw
                    )
        ...
        self.stdout.write(
            f"   ✅ Created {presets_created} timer presets, {matchups_created} matchups, {matches_created} matches"
        )

    def _seed_membership_plans(self, academies, athletes, plan_count):
        """Create membership plans and subscriptions."""
        self.stdout.write("💳 Creating membership plans and subscriptions...")
        ...
        for academy in academies:
            ...
            for name, plan_type, price, duration, class_limit in plan_configs:
                ...
                subscribing_athletes = random.sample(
                    academy_athletes, int(len(academy_athletes) * subscription_rate)
                )

                for athlete in subscribing_athletes:
                    # Only create 1 subscription per athlete per academy
                    if not Subscription.objects.filter(
                        athlete=athlete, plan__academy=academy
                    ).exists():
            ...
            for _ in range(random.randint(2, 5)):
                ...
                registering_athletes = random.sample(
                    academy_athletes,
                    min(
                        int(len(academy_athletes) * registration_rate), seminar.capacity
                    ),
                )
        ...
        self.stdout.write(
            f"   ✅ Created {plans_created} plans, {subscriptions_created} subscriptions, {transactions_created} tab transactions, {seminars_created} seminars"
        )

    def _seed_community(self, academies, athletes, achievement_count):
        """Create community achievements and open mat sessions."""
        self.stdout.write(
            f"🏆 Creating community features ({achievement_count} achievements)..."
        )
        ...
        for academy in academies:
            ...
            for week in range(8):
                saturday = start_date + timedelta(
                    days=(5 - start_date.weekday()) + (week * 7)
                )
                ...
                if not session.is_cancelled:
                    ...
                    rsvp_athletes = random.sample(
                        academy_athletes, int(len(academy_athletes) * rsvp_rate)
                    )

                    for athlete in rsvp_athletes:
                        status = random.choices(
                            ["GOING", "MAYBE", "NOT_GOING"], weights=[60, 25, 15]
                        )[0]

                        OpenMatRSVP.objects.create(
                            session=session, athlete=athlete, status=status
                        )
                        rsvps_created += 1

        self.stdout.write(
            f"   ✅ Created {achievements_created} achievements, {awards_created} athlete awards, {open_mats_created} open mats, {rsvps_created} RSVPs"
        )
